...
do
        ...
        cd ..
        slack_message.sh -t "Website deployed" -b "Successfully deployed website on server $SERVER in dir $DEPLOY_DIR_BASE$TRAVIS_REPO_SLUG/$TRAVIS_BRANCH from commit $TRAVIS_COMMIT. Check out http://$SERVER/deploy-info.txt" -c "repositories" -u "$SLACK_WEBHOOK_URL"
